...
const ProductsManage = () => {
  ...
  const staffCount = useMemo(() => products.filter(isStaffSourced).length, [products]);
  const customerCount = useMemo(() => products.filter((p) => !isStaffSourced(p)).length, [products]);
  ...
  const filtered = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchOrigin =
        originTab === "all" ||
        (originTab === "staff" && isStaffSourced(p)) ||
        (originTab === "customer" && !isStaffSourced(p));
      return matchSearch && matchCategory && matchOrigin;
    });
  }, [products, search, categoryFilter, originTab]);
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="!px-6 !py-10 text-center text-gray-400">ยังไม่มีสินค้า</td>
                </tr>
              )}
            </tbody>
          </table>
