...
const StaffProfile = () => {
  ...
  const displayName = profile?.name || localStorage.getItem("local_user_name") || "พนักงาน";
        ...
        <div className="flex flex-col items-center gap-4 border-b border-gray-100 !pb-6 text-center sm:flex-row sm:text-left">
          ...
          <div>
            ...
            <div className="mt-1.5 flex justify-center sm:justify-start">
              <StatusBadge
                label={ROLE_LABEL[profile?.role] || profile?.role || "พนักงาน"}
                color={profile?.role === "Admin" ? "purple" : "pink"}
              />
            </div>
          </div>
        </div>
