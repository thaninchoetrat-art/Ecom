...
export async function getMe(req, res, next) {
    try {
        const key = encodeEmail(req.user.email);
        const snapshot = await usersRef.child(key).get();
        const user = snapshot.val();
...
export async function createAccount(req, res, next) {
    try {
        ...
        await usersRef.child(key).set(newAccount);
...
export async function listAccounts(req, res, next) {
    try {
        ...
        const accounts = Object.values(all).map((u) => ({
            name: u.name,
            email: u.email,
            phone: u.phone || '',
